...
for word in $(<../ypm_list); do
    if [ -d "./ypm_plugins/${word}" ]; then
        ...
        bash -c "git submodule update --force ./ypm_plugins/${word} && cd ./ypm_plugins/${word} && ./build.sh && mv ./*.so ${YED_CONFIG_DIRECTORY}/ypm/plugins/${word}.so && cp -rf ./*.dSYM ${YED_CONFIG_DIRECTORY}/ypm/plugins/ 2> /dev/null" &
    fi
done
